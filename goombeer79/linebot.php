...
//関数実行
firebase($message_text,$userID);
sending_messages($accessToken, $replyToken, $message_type,countcheck($userID));
$count =countcheck($userID);
if($count >= 5){
  exit;
} elseif ($count < 5) {
  database_save($message_text,$userID,$count,countcheck($userID));
  push_messages($accessToken, $message_type,$userID,countcheck($userID));
}
